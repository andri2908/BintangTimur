...
namespace AlphaSoft
{
    public partial class dataInvoiceForm : Form
    {
        ...
        private void dataInvoiceDataGridView_DoubleClick(object sender, EventArgs e)
        {
            ...
            selectedRev = selectedRow.Cells["REV"].Value.ToString();

            switch (originModuleID)
            {
                case globalConstants.PEMBAYARAN_PIUTANG:
                    pembayaranPiutangForm pembayaranForm = new pembayaranPiutangForm(selectedSO, selectedRev);
                    pembayaranForm.ShowDialog(this);
                    break;

                case globalConstants.RETUR_PENJUALAN:
                    dataReturPenjualanForm displayedForm = new dataReturPenjualanForm(originModuleID, selectedSO, selectedRev);
        ...
        private void loadData()
        {
            ...
            if (originModuleID == globalConstants.RETUR_PENJUALAN)
                sqlCommand = "SELECT SALES_INVOICE AS 'NO INVOICE', MAX(REV_NO) AS REV, IFNULL(CUSTOMER_FULL_NAME, 'PELANGGAN UMUM') AS 'NAMA PELANGGAN', IF(SALES_PAID=1,'LUNAS','BELUM LUNAS') AS 'STATUS BAYAR' FROM SALES_HEADER H LEFT OUTER JOIN MASTER_CUSTOMER M ON " +
                                    "(H.CUSTOMER_ID = M.CUSTOMER_ID) WHERE 1 = 1 ";
            else if (originModuleID == globalConstants.PEMBAYARAN_PIUTANG)
                sqlCommand = "SELECT CREDIT.SALES_INVOICE AS 'NO INVOICE', MAX(REV_NO) AS REV, IFNULL(CUSTOMER_FULL_NAME, 'PELANGGAN UMUM') AS 'NAMA PELANGGAN', IF(SALES_PAID=1,'LUNAS','BELUM LUNAS') AS 'STATUS BAYAR' FROM SALES_HEADER H LEFT OUTER JOIN MASTER_CUSTOMER M ON " +
            ...
            using (rdr = DS.getData(sqlCommand))
            {
                if (rdr.HasRows)
                {
                    ...
                    dataInvoiceDataGridView.Columns["REV"].Visible = false;
