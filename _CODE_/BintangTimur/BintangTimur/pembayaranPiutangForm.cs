...
namespace AlphaSoft
{
    public partial class pembayaranPiutangForm : Form
    {
        ...
        public pembayaranPiutangForm(string SOInvoice, string SORev)
        {
            InitializeComponent();
        ...
        private void loadDataHeaderSO()
        {
            ...
            sqlCommand = "SELECT * FROM SALES_HEADER WHERE SALES_INVOICE = '" + selectedSOInvoice + "' AND REV_NO = " + selectedSORev;
        ...
        private void loadDataDetailSO()
        {
            ...
            sqlCommand = "SELECT D.PRODUCT_NAME AS 'NAMA PRODUK', PRODUCT_SALES_PRICE AS 'HARGA JUAL', PRODUCT_QTY AS 'QTY', SALES_SUBTOTAL AS 'SUBTOTAL' FROM SALES_DETAIL S, MASTER_PRODUCT D WHERE S.PRODUCT_ID = D.PRODUCT_ID AND S.SALES_INVOICE = '" + selectedSOInvoice + "' AND S.REV_NO = " + selectedSORev;
            using (rdr = DS.getData(sqlCommand))
            {
                if(rdr.HasRows)
                {
        ...
        private void calculateTotalCredit(bool globalCalculation = false)
        {
            ...
            { 
                ...
                {
                    globalTotalValue = Convert.ToDouble(DS.getDataSingleValue("SELECT SALES_TOTAL FROM SALES_HEADER WHERE SALES_INVOICE = '" + selectedSOInvoice+ "' AND REV_NO = " + selectedSORev));
                }
        ...
        public bool saveDataTransaction()
        {
            ...
            try
            {
                ...
                if (paymentMethod <= 3 && paymentNominal == globalTotalValue)
                {
                    ...
                    sqlCommand = "UPDATE SALES_HEADER SET SALES_PAID = 1 WHERE SALES_INVOICE = '" + selectedSOInvoice + "' AND REV_NO = " + selectedSORev;
        ...
        private void printReceipt()
        {
            ...
            sqlCommandx = "SELECT PC.PAYMENT_ID, SH.SALES_INVOICE, SH.SALES_TOTAL, SH.SALES_DISCOUNT_FINAL, SH.SALES_TOP, SH.SALES_TOP_DATE, PC.PAYMENT_DATE, PC.PAYMENT_CONFIRMED, PC.PAYMENT_CONFIRMED_DATE, PC.PAYMENT_NOMINAL, TAB2.PAID_AMT AS ACTUAL_PAYMENT " +
                                     ...
                                     "WHERE C.SALES_INVOICE = SH.SALES_INVOICE AND PC.CREDIT_ID = C.CREDIT_ID AND PC.PAYMENT_ID = TAB1.PAY_ID AND SH.SALES_INVOICE = '" + invoiceNoTextBox.Text + "' AND TAB1.CREDIT_ID = C.CREDIT_ID AND TAB2.CREDIT_ID = C.CREDIT_ID AND SH.REV_NO = " + selectedSORev;
        ...
        private void printDocument1_PrintPage(object sender, System.Drawing.Printing.PrintPageEventArgs e)
        {
            ...
            DS.mySqlConnect();
            //load customer id
            sqlCommand = "SELECT S.SALES_INVOICE AS 'INVOICE', C.CUSTOMER_FULL_NAME AS 'CUSTOMER',DATE_FORMAT(S.SALES_DATE, '%d-%M-%Y') AS 'DATE',S.SALES_TOTAL AS 'TOTAL', IF(C.CUSTOMER_GROUP=1,'RETAIL',IF(C.CUSTOMER_GROUP=2,'GROSIR','PARTAI')) AS 'GROUP' FROM SALES_HEADER S,MASTER_CUSTOMER C WHERE S.CUSTOMER_ID = C.CUSTOMER_ID AND S.SALES_INVOICE = '" + selectedSOInvoice + "' AND S.REV_NO = " + selectedSORev + " " +
                " UNION " +
                "SELECT S.SALES_INVOICE AS 'INVOICE', 'P-UMUM' AS 'CUSTOMER', DATE_FORMAT(S.SALES_DATE, '%d-%M-%Y') AS 'DATE', S.SALES_TOTAL AS 'TOTAL', 'RETAIL' AS 'GROUP' FROM SALES_HEADER S WHERE S.CUSTOMER_ID = 0 AND S.SALES_INVOICE = '" + selectedSOInvoice + "' AND S.REV_NO = " + selectedSORev + " " +
                "ORDER BY DATE ASC";
            using (rdr = DS.getData(sqlCommand))
            {
                if (rdr.HasRows)
